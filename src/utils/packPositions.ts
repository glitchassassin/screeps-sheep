...
const numberCodec = new Codec({ array: false, depth: 15 });
const cardinals = ["WN", "EN", "WS", "ES"];

/**
 * Pack a number to one Unicode character with screeps-utf15
 */
export const packNumber = (num: number) => {
  return numberCodec.encode(num);
};

/**
 * Unpack a number from one Unicode character
 */
export const unpackNumber = (str: string) => {
  return numberCodec.decode(str);
};
